                ...
                if sec.deduct_token(current_email):
                    with st.spinner("Auditing Resume..."):
                        try:
                            ...
                            """
                            response = client.models.generate_content(model="gemini-2.0-flash-lite", contents=[file_ref, prompt_strategy])
                ...
                if st.button("Draft New Resume (.docx)", key="btn_rewrite"):
                    with st.spinner("Rewriting Resume..."):
                         try:
                            ...
                            """
                            response = client.models.generate_content(model="gemini-2.0-flash-lite", contents=[file_ref, prompt_rewrite])
