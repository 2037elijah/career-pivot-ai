...
# We use a generic emoji for the tab icon to prevent "File Not Found" errors
st.set_page_config(
...
def check_password():
    ...
    if "password_correct" not in st.session_state:
        st.text_input(
            "🔑 Enter Access Code (From your Gumroad Receipt):", 
            type="password", 
...
if check_password():
    ...
    with col1:
        # We try to load the logo, but if it fails, we show nothing (Prevent Crash)
        try:
            st.image("logo_v2.png", width=250)
        except:
            st.write("🚀") # Show emoji if logo missing
            ...
            if st.button("Generate Strategy Report", key="btn_strategy"):
                with st.spinner("Auditing Resume..."):
                    try:
                        ...
                        Sections:
                        1. 🛑 ATS Audit (3 Red Flags)
                        2. 🔑 Skills Translation Table (Old vs New phrasing)
                        3. ✍️ Summary Rewrite
                        4. 🛠️ Cert Roadmap
                        ...
                        st.download_button("📥 Download HTML Report", html_report, "Strategy_Report.html", "text/html")
                        st.markdown("---")
                ...
                with st.spinner("Rewriting Resume..."):
                    try:
                        ...
                        with open(docx_file, "rb") as f:
                            st.download_button("📥 Download Word Doc", f, "Rewritten_Resume.docx", "application/vnd.openxmlformats-officedocument.wordprocessingml.document")
                        st.markdown("### Preview")
                        st.markdown(clean_response)
                        os.remove(temp_filename)
                    except Exception as e:
    ...
    with tab3:
            ...
            st.write("Now that you have the perfect resume, let's find the jobs.")
            
            # Input fields (Pre-filled if possible, but editable)
            col_a, col_b = st.columns(2)
            with col_a:
                target_role = st.text_input("Target Role", value="Data Center Technician")
            with col_b:
                target_location = st.text_input("Location", value="Charlotte, NC")
            
            # Generate Deep Links
            # We use %20 to represent spaces in URLs
            role_url = target_role.replace(" ", "%20")
            loc_url = target_location.replace(" ", "%20")
            
            linkedin_url = f"https://www.linkedin.com/jobs/search/?keywords={role_url}&location={loc_url}&f_TPR=r86400" # Last 24 hours
            indeed_url = f"https://www.indeed.com/jobs?q={role_url}&l={loc_url}&fromage=3" # Last 3 days
            google_url = f"https://www.google.com/search?q={role_url}+jobs+in+{loc_url}+hiring+now&ibp=htl;jobs"

            st.markdown("### 🎯 Instant Job Feeds")
            st.write("Click these buttons to open pre-filtered searches for the newest jobs:")
            
            c1, c2, c3 = st.columns(3)
            with c1:
                st.link_button("🔵 LinkedIn (Newest)", linkedin_url)
            with c2:
                st.link_button("⚪ Indeed (Recent)", indeed_url)
            with c3:
                st.link_button("🌈 Google Jobs", google_url)
            
            st.divider()
            st.info("💡 **Pro Tip:** Use the 'Resume Rewrite' from Step 2 to apply to these roles. It contains the exact keywords these algorithms are looking for.")

# --- SIDEBAR FEEDBACK MECHANISM ---
    with st.sidebar:
        st.divider()
        st.markdown("### 🗣️ Beta Feedback")
        ...
        if st.button("Submit Feedback"):
            # 1. VISUAL CONFIRMATION (Shows on the web page)
            st.warning(f"DEBUG MODE: Sending... {feedback_rating} Stars")
            
            # 2. LOGGING WITH FLUSH (Forces the server to print NOW)
            print(f"📝 NEW FEEDBACK: Rating={feedback_rating}, Comment='{feedback_text}'", flush=True)
